...
def check_of():
    try:
        ...
        ofs = odoo.execute('mrp.production', 'read', of_ids, ['name', 'state', 'workorder_ids', 'product_id'])
        
        print(f"Encontradas {len(ofs)} OFs recientes:")
        for of in ofs:
            print(f"\n--- OF: {of['name']} ({of['state']}) | Producto: {of['product_id'][1] if of['product_id'] else 'N/A'} ---")
